...
export default function CommunityPage() {
  ...
  // New state for controlling header visibility.
  ...
  // Listen to the window's scroll event to update header visibility
  useEffect(() => {
    const handleScroll = () => {
      const currentScrollTop = window.pageYOffset;

      // If scrolling up and not at the very top, hide the header.
      if (currentScrollTop < lastScrollTop.current && currentScrollTop > 0) {
        setShowHeader(false);
      } else {
        // When scrolling down or at the top, show the header.
        setShowHeader(true);
      }
    ...
    >
      ...
      <header
        style={{
          ...
          // Transition & transform to show/hide header based on scroll direction.
