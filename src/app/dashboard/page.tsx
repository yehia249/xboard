...
function DashboardContent() {
  ...
  />
      ...
      {/* Only render if there is at least ONE real subscription */}
      {subscriptions.length > 0 && (
        <div
          aria-label="Manage subscriptions"
          // Keep it below the header (header zIndex = 1000). Set this to 900 so header always stays on top.
          style={{
            position: "fixed",
            top: "94px",        // not too top-left (kept under sticky header)
            left: "12px",
            zIndex: 900,
            pointerEvents: "auto",
          }}
        >
          ...
          {subsOpen && (
            <div
              style={{
                marginTop: 10,
                width: "min(92vw, 420px)", // keep compact
                background: "#0b0b0b",
                border: "1px solid #222",
                borderRadius: 14,
                padding: 12,
                boxShadow: "0 10px 24px rgba(0,0,0,0.45)",
                maxHeight: "65vh",         // not too big
                overflow: "auto",
              }}
            >
