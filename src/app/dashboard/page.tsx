...
function DashboardContent() {
  ...
  // --- DEBUG: force-show a fake subscription panel (remove for production) ---
  const DEBUG_FORCE_SUBS =
    typeof window !== "undefined" &&
    (location.search.includes("debugSubs=1") || process.env.NODE_ENV !== "production");

  const MOCK_SUBS: PayNowSub[] = [
    {
      id: "sub_1234567890",
      status: "active",
      current_period_end: new Date(Date.now() + 1000 * 60 * 60 * 24 * 17).toISOString(),
      product: { name: "Gold Tier", metadata: { tier: "gold" } },
      plan: { name: "Monthly Gold", metadata: { tier: "gold" } },
      metadata: { tier: "gold" },
    },
  ];
  // --------------------------------------------------------------------------
  ...
  const cancelSubscription = async (id: string) => {
    ...
    // Optional (avoid network during debug)
    if (DEBUG_FORCE_SUBS) {
      showToast("success", "Subscription canceled", "Debug mode: no network call.");
      return;
    }
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ========= Manage Subscriptions (FLOATING top-left overlay) ========= */}
      {/* Only render if there is at least ONE subscription */}
      {subsForUI.length > 0 && (
        <section
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <h3 style={{ margin: 0, color: "#e5e7eb", fontSize: "1.05rem" }}>Manage subscriptions</h3>
              <button
      ...
      {/* (Removed the “No PayNow account” hint section entirely as requested) */}
